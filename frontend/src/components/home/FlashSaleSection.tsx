...
export default function FlashSaleSection() {
  ...
  return (
    <section id="flash-sale" aria-labelledby="flash-sale-heading" className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
      <div className="overflow-hidden rounded-md border border-gray-200 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 bg-white px-4 py-4 sm:px-5">
          <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
            <div>
              <h2 id="flash-sale-heading" className="flex items-center gap-1.5 text-base font-semibold uppercase tracking-wide text-orange-600">
                ...
                <div role="timer" aria-label={`${countdown[0]} jam ${countdown[1]} menit ${countdown[2]} detik`} className="flex items-center gap-1 text-sm font-semibold tabular-nums">
                  {countdown.map((value, index) => (
                    <span key={index} className="flex items-center gap-1">
                      ...
                      <span className="flex h-6 w-6 items-center justify-center rounded bg-slate-800 text-white">{String(value).padStart(2, '0')}</span>
                    </span>
                  ))}
                </div>
              </div>
          ...
          <div className="flex gap-2">
            {([-1, 1] as const).map((direction) => (
              <button key={direction} type="button" aria-label={direction === -1 ? 'Produk sebelumnya' : 'Produk berikutnya'} aria-controls="flash-sale-products"
                ...
                className="flex h-9 w-9 cursor-pointer items-center justify-center rounded border border-gray-200 bg-white text-slate-600 hover:border-orange-300 hover:text-orange-600 focus-visible:outline-2 focus-visible:outline-orange-600">
        ...
        <ul id="flash-sale-products" ref={list} tabIndex={0} aria-label="Produk flash sale" className="flex snap-x snap-mandatory gap-3 overflow-x-auto p-4 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-orange-600 sm:gap-4 sm:p-5">
          {flashSaleProducts.map((product) => (
            <li key={product.id} className="w-40 shrink-0 snap-start overflow-hidden rounded border border-gray-100 sm:w-48">
              ...
              <div className="p-3">
                ...
                <p className="mt-2 text-lg font-semibold text-orange-600">{rupiah.format(product.price)}</p>
